...
public:
    String() : m_data(NULL), m_size(0), m_capacity(0) {
        reserve(1);
    }

    String(const char* s) : m_data(NULL), m_size(0), m_capacity(0) {
        if (!s) {
            reserve(1);
            return;
        }
    ...
    String(const char* s, size_t n) : m_data(NULL), m_size(0), m_capacity(0) {
        reserve(n + 1);
        if (s && n > 0)
            ::memmove(m_data, s, n);
        m_size = n;
    ...
    String(const String& other) : m_data(NULL), m_size(0), m_capacity(0) {
    ...
    void resize(size_t n) {
        reserve(n + 1);
        if (n > m_size)
            ::memset(m_data + m_size, 0, n - m_size);
        m_size = n;
        m_data[m_size] = '\0';
    }
    ...
    String& operator+=(const String& rhs) {
        if (rhs.m_size == 0)
            return *this;
        reserve(m_size + rhs.m_size + 1);
        ::memmove(m_data + m_size, rhs.m_data, rhs.m_size);
        m_size += rhs.m_size;
        m_data[m_size] = '\0';
        return *this;
    }
...
inline void push_vlq_uint(size_t n, String& out) {
...
inline u32 fnv32a(const unsigned char* i, size_t len, u32 hash = 0x811c9dc5) {
...
inline void pack_bytes(const unsigned char* i, u16& packed, size_t blocksize) {

    u32 a = fnv32a(i, MIN_RUN);
...
struct offsets_dict_t {
    ...
    typedef Vector<size_t> offsets_t;
    offsets_t offsets;
    ...
    offsets_dict_t(size_t sl, size_t bs) : searchlen(sl), blocksize(bs) {
        // Pre-allocate the full hash table once so clear() never needs to reallocate.
        offsets.assign((searchlen + 1) * blocksize, 0);
    }

    void clear() {
        // Zero the existing allocation in-place — no heap alloc/free per compression call.
        ::memset(offsets.data(), 0, (searchlen + 1) * blocksize * sizeof(size_t));
    }
    ...
    void operator()(u16 packed, const unsigned char* i0, const unsigned char* i, const unsigned char* e,
...
struct compress_t {
    ...
    String feed(const unsigned char* i, const unsigned char* e) {
...
struct decompress_t {
    ...
    bool feed(const String& s, String& remaining) {
    ...
    bool feed(const unsigned char* i, const unsigned char* e, String& remaining) {
        ...
        if (state.state == state_t::INIT) {
            ...
            {
                OSReport("Decompressed size %d exceeds maximum size %d\n", size, max_size);
            }
        ...
        while (i != e) {
            ...
            if (state.state == state_t::READ_DATA) {
                ...
                if (out + len > oute)
                    OSReport("Malformed data while uncompressing\n");
