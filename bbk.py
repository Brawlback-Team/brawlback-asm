...
def _import_err(e: ImportError):
    sys.stderr.write(f'missing python modules: {e}\n'
...
def setup(redownload: bool):
    ...
    if not kuribo_dir.exists() or not kuribo_dir.is_dir():
        ...
        if not (kuribo_arches := kuribo_vers.get(KURIBO_LLVM_VER)):
            LOG.error(f"toolchain mirror missing required kuribo-llvm version {KURIBO_LLVM_VER}")
            sys.exit(1)

        if not (kuribo_uri := kuribo_arches.get(host_arch)):
            LOG.error(f"toolchain mirror missing a kuribo-llvm build for your computer's architecture ({host_arch})")
            sys.exit(1)

        LOG.info(f'found required kuribo-llvm toolchain ({kuribo_uri})')
        with Progress() as progress:
            with tempfile.TemporaryFile() as df:
                download_url(progress, progress.add_task('downloading kuribo-llvm'), f'{TOOLCHAINS_URI}/{kuribo_uri}', df)
                df.seek(0)

                tf = tarfile.open(fileobj=df)
                members = tf.getmembers()
                decompress_task = progress.add_task('decompressing kuribo-llvm archive', total=len(members))
    ...
    if not elf2rel.exists() or not elf2rel.is_file():
        ...
        if not (elf2rel_arches := elf2rel_vers.get(ELF2REL_VER)):
            LOG.error(f"toolchain mirror missing required elf2rel version {ELF2REL_VER}")
            sys.exit(1)

        if not (elf2rel_uri := elf2rel_arches.get(host_arch)):
            LOG.error(f"toolchain mirror missing an elf2rel build for your computer's architecture ({host_arch})")
            sys.exit(1)

        LOG.info(f'found elf2rel binary ({elf2rel_uri})')
        with Progress() as progress:
            with open(elf2rel, 'wb') as f:
                download_url(progress, progress.add_task('downloading elf2rel'), f'{TOOLCHAINS_URI}/{elf2rel_uri}', f)
